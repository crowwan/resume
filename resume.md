...
프론트엔드 아키텍처 설계와 자동화 경험을 바탕으로, 제품 품질과 개발 효율을 동시에 개선해온 3년차 개발자입니다.
B2B SaaS 환경에서 Micro Frontend 전환, 모노레포 통합, E2E 테스트 자동화 등 플랫폼 전반의 기술적 의사결정을 주도하고 있습니다.
...
리팩토링이나 마이그레이션처럼 코드를 더 나은 방향으로 정리하는 일에 자연스럽게 동기부여를 받습니다.
주도적으로 테스트와 자동화를 도입하여 팀의 기반을 다지고, 긍정적인 개발 문화를 전파하는 사람이 되고 싶습니다.
...
**주요 성과**
- 분산된 **5개+ 프론트엔드 프로젝트**를 NX 모노레포로 통합, **CI/CD** 및 **Datadog 모니터링** 전사 적용
- Playwright E2E **80+ TC** 구축, Page Object 패턴으로 로그인 중복 코드 **93% 감소**, 세션 공유로 실행 성능 **39% 개선**
- **K8s CronJob** 일일 자동 테스트 환경 구축, **Claude Code CLI** 기반 AI 변경 감지 → 관련 테스트 선별 실행
- 레거시 마이그레이션으로 TS 에러 **688→0개** 해결, Facade 패턴 토큰 관리 **200→33줄(85%↓)**, 도메인 **33→10개(70%↓)** 통합 설계
- **Export v5 API** 마이그레이션 및 crown-io 리팩토링, **Viewer Module** 고도화 (3D 뷰어, Case Share, 모바일 최적화)
...
iframe 기반 MFE 5개 모듈 운영의 한계를 경험하고, Module Federation 기반 아키텍처로 전환하여 독립 배포 체계와 중앙화된 모듈 관리를 확립.

**주요 성과**
- **iframe MFE 5개 모듈** 운영, 외부 스캐너 **3개 서비스** OAuth 연동 및 서브 도메인 마이그레이션 **무중단 전환** 완료
- **Module Federation** 호스트-리모트 아키텍처 설계, **FSD** 도입 및 ESLint 커스텀 규칙으로 레이어 간 import 방향 강제
- 사내 백오피스용 **4개 원격 모듈**을 3주 내 신규 개발 (87 커밋, **58K+ LOC**)
- 호스트-리모트 간 **타입 공유 자동화** 스크립트 구현, **MSW** Mock API 환경 구축
...
**주요 성과**
- **SaaS 구독 관리 시스템** 전체 설계/구현 (Free/Paid 플랜, 일할 계산, 시트 구매, 결제 관리, 구독 취소/재개, 쿠폰)
- **Seats→Members** 시스템 전환 리팩토링, 멤버 초대/권한 관리 (다지역 처리 포함)
- OAuth 플로우에 글로벌 **Storage Location** 선택 및 **MFA 인증** 구현, 글로벌 서비스 대응
- **3단계 반응형 UI** 시스템 설계 (모바일/태블릿/데스크톱), **Jest + MSW** 테스트 인프라 구축
...
**주요 성과**
- **Electron + Vite + React** 데스크톱 앱 0→1 설계, 내장 Express 서버 통신, **Config 클래스** 기반 동적 SW 관리, v1.0.0~v1.0.2 릴리즈
- **16개+ CAM 소프트웨어**별 좌표계 차이를 **변환 매트릭스 알고리즘**으로 해결, Export Session 방식 Protocol Handler 설계
- **Chrome PNA** 규제 선제 대응 (WebSocket/Protocol POC → dentbird-linker:// 채택), 인스톨러 **854MB→78MB (91%↓)**
- **Azure Pipelines** Windows/macOS 크로스 플랫폼 빌드 자동화 (**EV 코드 서명** 포함), **Datadog** RUM/Logs 모니터링 통합
...
**주요 성과**
- **Electron + React + TypeScript** 데스크톱 앱 초기 세팅부터 구축, 라우팅/SNB/페이지 공통 컴포넌트 등 프로젝트 구조 설계
- **IPC 통신 아키텍처** 설계, **Deep Link**(dentbird://) 커스텀 프로토콜 및 **자동 업데이트** 시스템 구현
- **Webpack** Main 프로세스 번들링, Windows/Mac **코드 서명**, 환경별(dev/qa/prod) 빌드 분리, Artifact 버전 관리까지 빌드/배포 파이프라인 구축
- 테스트 문화 부재 환경에서 **Jest + MSW** 단위 테스트 인프라 도입, 주요 모듈 테스트 코드 작성
- **Datadog RUM** 연동 (환경별 설정 분리, 빌드 시 환경변수 주입) 및 **Crash Reporter** 구축, 프로덕션 에러 추적 체계 확립
...
- **i18next + TypeScript** 타입 안전한 다국어 시스템 설계, **3개 언어**(한/영/일) 지원, 타입 자동 생성 패키지 도입
- **LCP 이미지 프리로드**, Next.js Image 최적화, 캐싱 전략으로 **Lighthouse 성능 점수** 개선
- **Backoffice 조직 관리** 3계층 CRUD를 1주 53커밋으로 구현, **Fastify/MongoDB** 기반 일괄 수정 API 설계/개발
