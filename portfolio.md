...
클라이언트 코드에 **구체적인 컨벤션이 부족했습니다** — 언제 훅으로 빼고 언제 컴포넌트로 둘지, 파일을 어디 두고 중복을 어떻게 처리할지에 대한 명확한 기준이 없었습니다. `components`·`hooks` 같은 **역할 단위 폴더**라 만들기는 쉬웠지만, 그 훅이 어디서·왜 쓰이는지 한눈에 안 보였고 **같은 목적의 코드가 흩어져** 있었습니다. 폴더 구조를 여러 번 바꿔 봐도 기준이 없으니 곧 원형으로 돌아갔고, 에러 처리 기준도 없어 **한 기능의 에러가 전체 페이지를 죽이고 전부 unknown으로 뭉개졌습니다.**
...
- **FSD를 도입했는데도 "이 파일을 어디 넣지"가 안 풀렸습니다.** `entities`·`shared`·`features`는 명확했지만 `widgets`·`pages`처럼 **UI 단위가 레이어에 섞여** 한 기능의 UI를 features에 둘지 widgets에 둘지 애매했고, 그걸 정하느라 작은 논의가 끊이지 않았습니다 — 위치 고민을 줄이려 도입했는데, 정작 그 지점을 못 풀었습니다.
...
그래서 지금은 기술을 도입할 때 **가장 우선순위 높은 문제를 먼저 정의**하고, 대안을 비교해 맞을 때만 **팀이 같은 이해를 가진 상태에서** 도입합니다. 효용만이 아니라 **도입 비용(커뮤니케이션·마이그레이션·과도기 복잡도)까지 저울질**해야 한다는 것도 이때 배웠습니다. 구조 자체가 나빴던 건 아닙니다 — 그 코드는 이후 모노레포로 옮겨가서도 그대로 살아남았습니다. 배운 건 '좋은 구조'가 아니라 **'도입의 전제'**였습니다.
...
케이스 목록은 사용자가 가장 자주 보는 화면이고, 썸네일이 수십 장 깔립니다. "N장 = 네트워크 N요청"이 되기 쉬운 이 화면의 **썸네일 로딩 성능**과, 운영 중 터진 **깜빡임·무한 재요청 회귀**를 다룬 작업입니다.
...
일회성 크레딧에서 **반복 구독으로 결제 모델을 전환**하는 시점에 구독·결제 화면을 맡았습니다 — 플랜 업그레이드, 좌석(시트) 구매, 구독 취소·재개, 쿠폰, 결제수단 관리, 결제 히스토리. 결제는 틀리면 곧바로 신뢰가 깨지는 영역이라, 화면을 만드는 데서 그치지 않고 **출시 뒤 운영 버그와 회귀 가드까지 오래 맡았고**, 외부(팝업·Stripe·OAuth)와 주고받는 경계에서 무너지지 않게 하는 데 집중했습니다. (결제 인프라·팝업 앱 기반은 팀과 함께였습니다.) 나아가 **부분 실패에도 무너지지 않는 의존 관계 설계를 제안**했습니다(착수 전).

### 문제

결제는 우리 서버·Stripe·결제 팝업이 계속 주고받으며 돌아가는 화면이라, 그 사이가 한 군데만 어긋나도 사용자가 막힙니다. 실제로 서버가 **HTTP 200인데 본문에 비즈니스 에러**를 담아 주거나, 결제 정보 일부를 못 받았다고 화면 전체가 죽으면 구독 취소·내역 조회조차 못 하게 됩니다.

### 해결 — 외부와 주고받는 경계를 하나씩

결제는 **별도 팝업 창**(팀과 함께 만든 결제 앱)에서 Stripe로 진행하고, 끝나면 팝업이 부모 창에 결과를 `postMessage`로 돌려줍니다. 이때 아무 창이나 받지 못하도록 **보낸 곳(referrer) origin에만** 전달하게 했습니다. 그런데 운영 중 **결제를 마치면 'Unknown error' 화면이 뜨는 P1**이 있었습니다 — 팝업이 부모에 메시지를 보낼 때 target origin이 빈 문자열이라 막힌 것이었고, 추적해 보니 **팝업 앱 빌드가 루트 환경설정(.env)을 안 읽어** origin 값이 비어 있었습니다. 빌드 설정을 고쳐 해결했습니다.

업그레이드·취소 같은 변경은 Stripe·서버 동기화가 끝나야 화면에 반영돼서, 끝날 때까지 상태를 **폴링**합니다. 이게 무한히 돌던 것을 **3초 간격·최대 20회, 화면을 벗어나면 정리**하도록 막았습니다.

외부 스캐너 계정 연동(OAuth)에서도 비슷한 결함을 잡았습니다. 토큰 인터셉터가 모듈 상태의 토큰으로 요청 헤더를 무조건 덮어써 로그아웃 후 재로그인 시 옛 토큰으로 401이 나던 결함을 **요청 단위 토큰을 우선하도록** 고쳤고, '동시 호출 방지' 안전장치가 콜백 예외 시 잠금을 영영 못 풀어 무한 로딩에 빠지던 문제를 `try-finally`로 바로잡았습니다 — **안전장치가 자기 결함으로 죽지 않게** 한다는, Case 4와 같은 원칙입니다. 외부 스캐너 OAuth 콜백이 간헐적으로 깨지던 문제는 1차 수정(중복 실행 멱등 처리)이 dev에서 재발하자, **콜백 화면이 앱 부트스트랩 트리 안에 있어 리마운트마다 가드가 리셋되는 구조**를 원인으로 확정하고 콜백을 트리 밖 경량 셸로 분리해 풀었습니다.
