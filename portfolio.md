...
> **Dentbird의 프론트엔드를 맡아 복잡한 화면·3D 뷰어부터, 그 코드를 받치는 아키텍처·디자인 패턴·상태관리, 나아가 모노레포·빌드·품질 자동화 같은 플랫폼 토대까지 담당했습니다.**
>
> 화면을 만들 때는 FSD·컴포넌트 패턴·선언적 에러 처리로 오래 버티는 구조를 고민했고, 리스트 렌더링 성능과 URL 상태 같은 디테일을 챙겼으며, 부분 실패에도 사용자가 막히지 않도록 장애 허용 범위를 설계했습니다. 동시에 브라우저 보안 정책에 막힌 웹↔로컬 연동을 Custom Protocol로 재설계하고, 담당자 PC에 묶인 데스크톱 빌드를 자동화하고, 테스트가 거의 없던 제품에 격리 재현 환경을 만들었습니다.
>
> 기술은 당장 빠른 것보다 오래 버티는 것을 기준으로 고르려 했고, 한 번 정한 뒤에도 운영하며 한계를 만나면 다시 설계했습니다. 그 과정에서 되돌린 결정과 도입하지 않은 결정, 그리고 패턴을 잘못 도입하며 배운 것까지 정직하게 적었습니다.
...
계정·구독 클라이언트를 입사 초기부터 오래 맡으며, 프론트엔드 아키텍처와 디자인 패턴을 본격적으로 도입하고 **그 한계까지 겪은** 프로젝트입니다. 결론부터 말하면, 이 케이스의 핵심은 "패턴을 적용했다"가 아니라 **"패턴을 언제·왜 도입해야 하는지"를 배운 과정**입니다.

### 문제

클라이언트 코드가 구조 기준 없이 커지면서, 응집도는 낮고 결합도는 높아졌습니다. 요구사항이 바뀔 때 어느 파일을 고쳐야 할지, 고치면 어디에 사이드이펙트가 생길지 장담할 수 없었습니다. 도메인 정의가 없어 문제 구조가 한눈에 안 들어왔고, 앱 서비스 로직과 비즈니스 로직이 섞였으며, 에러 처리도 정의돼 있지 않아 **한 기능에서 난 에러가 전체 페이지를 에러 화면으로 바꾸고 전부 unknown으로 뭉개졌습니다.**

### 해결 — 구조·패턴·에러 처리를 함께 정리

- **FSD(Feature-Sliced Design)**로 폴더 역할을 명확히 하고 도메인 데이터를 분리했습니다. API는 `{Domain}API` 객체 + TanStack Query query key factory로, DTO와 도메인 객체는 mapper로 분리하는 규칙을 직접 정립했습니다.
- **Compound Component / Render Props**로 컴포넌트 관심사를 분리했습니다. 각 패턴의 장점만이 아니라 단점(Context 리렌더 비용, "커스텀 훅+props로도 가능하다")까지 함께 기록하고, **관심사 분리가 목적이 아니면 굳이 쓰지 않는다**는 판단 기준을 세웠습니다.
- **선언적 에러 처리**로 전환했습니다. 명령형 `if (!data) return null` 방어 코드를 ErrorBoundary 기반으로 바꾸되, "모든 컴포넌트에 ErrorBoundary를 쓸까? 아니다"라고 판단해 **장애 허용 범위(Fault Tolerance)를 어디까지 둘지를 화면 단위로 직접 설계**했습니다. 비즈니스 에러는 `BusinessLogicError` 커스텀 클래스로 `statusCode 200 / success false / errorCode`를 throw해 코드별로 분기했습니다.
...
또 다른 적용 예는 **기능 플래그로 두 구현을 전환하는 자리**입니다. 통신 방식(Custom Protocol ↔ HTTP)을 플래그로 가르면서, 나중에 한쪽으로 굳히기 쉽도록 **전환 지점 자체를 Render Props로 추상화**했습니다. 이때 "플래그로 켜고 끄는 코드라 공통 로직의 약간의 중복은 의도적으로 허용한다"고 판단해, **추상화의 강도를 문제에 맞춰** 골랐습니다 — 패턴을 '쓰는 법'이 아니라 '얼마나 쓸지'를 고른 사례입니다. (팀에서 기능 플래그 활용을 넓혀가는 중입니다.)
...
- **FSD를 도입했는데도 "이 파일을 어디에 넣어야 하지"를 매번 상의**하느라 시간이 많이 들었습니다. 위치 고민 없이 파일을 만들 수 있어야 좋은 구조라고 생각하는데, 실제로는 그렇지 못했습니다.
- **디자인 패턴을 "코드 일관성"이라는 명분으로 굳이 안 써도 되는 곳에 적용**한 경우가 많았고, 제가 제안한 방향을 팀원들이 다르게 이해해 같은 패턴인데 쓰는 방법이 제각각이 되기도 했습니다.
- 가장 크게 깨달은 건 — 디자인 패턴은 원래 오랜 기간 누적된 공통 문제를 푸는 해법인데, 저는 **그 패턴이 어떤 문제를 푸는지 정확히 이해하지 못한 채 도입**했다는 것입니다.

### 회고

그래서 지금은, 기술을 도입할 때 항상 **우선순위가 가장 높은 문제가 무엇인지를 먼저 정의**하고, 패턴은 그 문제에 맞을 때만, **팀이 같은 이해를 가진 상태에서** 도입해야 한다고 생각합니다. 구조 자체가 나빴던 건 아닙니다 — 그 코드들은 이후 모노레포로 옮겨가면서도 그대로 살아남았고, 좋은 형태라는 생각엔 지금도 변함이 없습니다. 배운 건 '좋은 구조'가 아니라 **'도입의 전제'**였습니다.

---

## Case 2. 케이스 목록을 빠르고 매끄럽게 — 리스트 렌더링 성능과 URL 상태

`2025.10 ~ 2026.06` · `React · TanStack Query · IntersectionObserver · Context API`

### 개요

케이스 목록은 사용자가 가장 자주 보는 화면입니다. 썸네일이 수십 장 깔리고, 목록과 3D 리뷰를 오가는 토글이 있습니다. 이 화면의 **렌더링 성능**과 **URL 기반 상태 동기화** 두 가지를 다룬 작업입니다.

### 썸네일 — 보일 때만, 모아서 한 번에

케이스 목록 썸네일은 "N장 = 네트워크 N요청"이 되기 쉽습니다. **IntersectionObserver로 viewport에 들어온 썸네일만 로드**하고, 50ms 윈도우 안의 요청을 모아 **한 번에 배치 요청**하는 공통 훅(Context Provider 기반)을 만들었습니다.

운영 중 깜빡임과 무한 재요청 회귀가 터지자 보강했습니다 — URL을 **TTL 캐시**(성공 4분·실패 10초)하고, 로딩 상태는 첫 fetch에만 토글하고 재시도는 silent로 두었으며, `<img>`의 `onError → 재fetch → 또 403`이 무한 반복하던 것을 재시도 1회로 막았습니다. 이때 **"성공하면 errorCount를 리셋할까"를 검토했다가, 같은 영구 오류가 다시 무한 루프를 만든다는 이유로 일부러 리셋하지 않는** 결정을 코드 주석으로 남겼습니다.

![리스트 렌더링 성능과 URL 상태 — 보일 때만 로드하고, 내 키만 비교한다](images/portfolio/diagrams/list-performance.svg){.diagram}

### 목록 ↔ 3D 리뷰 토글 — URL을 상태로

목록과 3D 리뷰를 오가는 토글이 누르자마자 즉시 되튕기는(bounce-back) 버그가 있었습니다. 원인은 URL→state 동기화가 **전체 query string을 diff**해 외부 변경을 판단하던 것이었습니다. 다른 훅이 `reviewCase`를 URL에 쓰는 순간 그게 "외부 변경"으로 오판돼 stale 값으로 되돌아갔습니다.

여러 훅이 같은 URL을 공유하는 환경에서 **"내 키만 본다"**로 비교 범위를 좁혀(own-key 부분집합 diff) 타 owner 키 변경을 무시하게 했습니다. 이산적인 키(view·tab)는 즉시 push하고 연속 입력(검색어)만 debounce를 유지해, race를 타이머가 아니라 **비교 경계로 근본 해결**했습니다.

### 회고

리스트 화면은 "그냥 보여주면 되는" 화면처럼 보이지만, 성능(네트워크·렌더)과 상태(URL·race)가 가장 많이 부딪히는 곳입니다. 두 문제 모두 "더 빨리·더 많이"가 아니라 **"안 해도 되는 일을 안 하게"**(보일 때만 로드, 내 키만 비교) 만드는 쪽으로 풀었습니다.
