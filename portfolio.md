...
- 빌드 시간을 **20분 → 6분(직접 비교)** 으로 단축하고, 빌드의 담당자·인프라팀 의존 제거
...
원인은 환경변수를 **빌드타임에 박아 넣는 구조**였습니다. 이를 **런타임 주입**으로 바꾸는 작업을 팀과 공동 설계했고, 그 결과 같은 빌드 결과물 하나를 dev·qa·prod에 그대로 올리고 환경변수만 갈아끼우는 방식이 가능해졌습니다.
...
- 환경 차이로 깨지던 E2E를 안정적인 기반 위로 옮겼고, 이 위의 E2E 자동화에서 **로그인 셋업 중복 93% 제거·스위트 18분 → 11분(39%↓)** 으로 이어졌으며, 이후 AI 변경 감지 자동화의 실행 기반으로 확장됐습니다
...
EC2에 Claude를 상주시켜, 커밋의 변경 내용(diff)을 분석하고 **변경과 관련된 E2E만 선별해 실행**한 뒤 결과를 Teams로 보고하는 파이프라인을 설계했습니다. 검증을 10분 주기로 자동화하고, Lockfile로 동시 실행을, state tracking으로 중복 분석을 막아 안정적으로 돌도록 만들었습니다.

```mermaid
flowchart LR
  Commit["커밋 변경<br/>(diff)"]
  Claude["EC2 Claude<br/>변경 분석"]
  Select["관련 E2E<br/>선별"]
  Run["격리 환경에서<br/>선별 실행"]
  Teams["Teams 보고"]
  Commit --> Claude --> Select --> Run --> Teams
```

### 결과 (RESULT)

- 전수 실행 대신 **변경 연관 테스트만 선별 실행**해 피드백 주기를 앞당기고, 회귀를 **같은 PR 안에서 조기 차단**
- 10분 주기 무인 검증으로, 사람이 매번 "어떤 테스트를 돌릴지" 판단하던 수동 작업을 제거
- (선별 정확도·실행량 절감의 정량 효과는 측정 근거가 확보되면 보강 예정 — 현재는 메커니즘과 운영 방식까지만 기재)
