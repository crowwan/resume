...
여러 통신 방식을 비교하고, 구현 속도가 아니라 장기 안정성을 기준으로 선택했습니다.

| 방안 | 검토 결과 |
|------|-----------|
| WebSocket 로컬 서버 | 구현이 가장 빠르나, "곧 LNA 적용 예정"이라 1~2년 후 재차단될 단기 해법 → **탈락** |
| HTTPS 로컬 서버 | 인증서·배포 등 별도 부담 |
...
명확한 스펙이 없던 외부 연동은, 기존 export 기능을 **특성화 테스트로 고정하고 구현으로 통과**시키는 전략을 썼습니다.

빌드·배포 인프라도 함께 재설계했습니다. 담당자 PC와 인프라팀(코드사인 USB)에 묶여 있던 구조를, **Windows 코드 서명 에이전트를 물리 빌드머신에 직접 설치해 단독 운영하고 GitHub Actions로 이관**해 끊어냈습니다. 개발 중 *코드 서명을 빌드 뒤 따로 하면 자동 업데이트가 깨진다*는 걸 직접 부딪혀 배우고, 서명을 빌드 과정에 포함하는 구조로 정리했습니다.

### 결과 (RESULT)

- 권한 팝업을 놓쳐 발생하던 통신 차단 CS 유형을 **구조적으로 제거**
- 자체 로컬 서버가 없는 CAM까지 연동 범위 확장
- macOS 빌드를 **33~39분 → 17~24분(약 56%↓, PR 실측)**으로 단축, artifact **757MB → 334MB**로 줄이고, 빌드의 담당자·인프라팀 의존 제거
...
**Module Federation**도 후보였습니다. 다만 다른 기능에 도입해본 경험상 초기 설정이 복잡하고 모노레포가 아닌 소비처에서 설정 부담이 커서 이 건에서는 제외했습니다 — 몰라서가 아니라 **써보고 트레이드오프로 뺀** 판단입니다. (console-client에는 Module Federation을 직접 적용했습니다.)
...
초기 4개 공통 모듈의 **iframe 런타임 통합을 주도**했고, console-client의 Module Federation 설정을 담당했습니다. 사내 3D 라이브러리(iwtk)→Three.js 전환은 팀이 AI 기반으로 주도(vtk vendor lock-in 해소)했고, 본인은 그 위에서 **렌더 정합을 소스 엔진(iwtk)의 실제 동작 기준으로 다시 잡고 회귀를 가드**(현재 진행)합니다. (번들 절감 수치는 팀 차원의 전환 결과로, 본 케이스의 정량 성과로 포함하지 않습니다.)
...
모노레포 이관 중, dev·qa에서는 멀쩡하던 것이 **prod 배포 시 dev 환경변수가 박힌 채 배포**되는 일이 있었습니다(팀이 풀던 런타임 분리 문제의 배경). 그 결과 배포 직후 곧바로 핫픽스를 다시 배포하는 일이 반복됐습니다. 동시에 특정 시점의 버그를 재현하기 어려웠고, E2E가 다른 변경의 간섭으로 쉽게 흔들렸습니다.

### 접근 (HOW) — 선행 구조 먼저

원인은 **빌드 시점에 환경변수가 번들에 박히는 구조**였습니다. **런타임 분리 자체는 팀이 진행**했고(빌드타임→런타임 주입), 본인은 그 토대로 **도메인 통합 라이브러리(urlHelper)** 를 깔았습니다 — 기준 도메인 하나에서 backend·gateway·테넌트 호스트 등 수십 개의 URL·도메인이 한 곳에서 자동으로 파생되도록 환경별 설정을 일원화한 것이고, 이 토대 위에서 팀의 런타임 config·격리 환경 작업이 진행됐습니다.

이 구조가 다음 단계를 열었습니다. 환경이 런타임에 결정되니, **특정 커밋 시점으로 Docker를 띄워 그때의 클라이언트+서버 환경을 그대로 재현**하는 격리 환경을 쌓을 수 있었습니다. 핵심은 격리 환경을 바로 만든 게 아니라, 도메인 통합·런타임 config로 *"격리하기 쉬운 환경"* 을 먼저 깔고 그 위에 쌓았다는 점입니다. 이 격리 환경은 **다른 변경의 간섭 없이 특정 시점을 결정론적으로 재현**하는 것을 목표로 합니다.
...
- 만성적으로 실패하던 격리 daily를 원인별로 분리 진단해 근본 수정(**결제 테스트 누락 약 28건 → 0**)
- 다른 변경의 간섭으로 흔들리던 E2E를 안정적인 기반 위로 옮겼고, 이 인프라가 이후 E2E·AI 변경 감지 자동화의 실행 기반으로 확장

### 내 기여 범위

런타임 분리 자체는 **팀**이 진행했고, 본인은 **도메인 통합 라이브러리(urlHelper)로 그 토대를 제공**(환경별 URL·도메인 일원화)했습니다. 그 위의 **격리 재현 환경 구축·만성 실패 근본 수정**은 본인이 담당했습니다(첫 기동 단순화, 격리 E2E 회귀 가드, 로컬 SQS 에뮬레이터, 격리 daily 원인별 진단·수정 등).
...
EC2에 Claude를 상주시켜, 커밋 diff를 분석하고 **QA팀이 관리하는 qase 테스트 케이스 중 연관 케이스를 선별·실행**한 뒤 결과를 Teams로 보고하고, **실패를 실제 회귀와 테스트 코드 문제로 1차 분류**하는 파이프라인을 설계했습니다. 검증을 10분 주기로 자동화하고, 동시 실행·중복 분석을 막아 안정적으로 돌도록 했습니다.
...
- 전수 실행 대신 **변경 연관 qase TC만 선별 실행**해 피드백 주기를 앞당기고, 회귀를 조기에 탐지
- 사람이 매번 "어떤 테스트를 돌릴지" 판단하던 수동 작업을 제거하고, 실패를 실제 회귀와 테스트 코드 문제로 1차 분류
