...
계정·구독 클라이언트를 입사 초기부터 오래 맡았는데, 클라이언트 코드에 **구체적인 컨벤션이 부족했습니다** — 언제 훅으로 빼고 언제 컴포넌트로 둘지, 파일을 어디 두고 중복을 어떻게 처리할지에 대한 명확한 기준이 없었습니다. `components`·`hooks` 같은 **역할 단위 폴더**라 만들기는 쉬웠지만, 그 훅이 어디서·왜 쓰이는지 한눈에 안 보였고 **같은 목적의 코드가 분산돼** 있었습니다. 폴더 구조를 여러 번 바꿔 봐도 기준이 없으니 곧 원형으로 돌아갔습니다.

### 해결 — 구조·패턴을 함께 정리

마침 구독제로 전환하며 **기존 로직과 분리된 기능을 새로 시작할 기회**가 생겨, 팀과 공감대를 이뤄 처음으로 FSD를 도입했습니다.

- **FSD**로 폴더 역할과 도메인 데이터를 분리하고, API는 `{Domain}API` + query key factory, DTO와 도메인은 mapper로 가르는 규칙을 정립했습니다.
...
이 기준을 적용한 예가 둘 있습니다. **Compound Component** — 3개 앱에 중복되던 CaseInfo 다이얼로그를 Compound + FormProvider 단일 컴포넌트로 추출해 약 66%(984→330줄) 줄였습니다. **Render Props** — 통신 방식(Custom Protocol과 HTTP)을 나중에 한쪽으로 굳히기 쉽도록 전환 지점만 추상화하되, 약간의 중복은 의도적으로 허용해 *얼마나 추상화할지*를 문제에 맞춰 골랐습니다.
...
에러 처리는 프론트 혼자 정할 수 없고 서버와의 계약이 전제였습니다. 에러 응답 형식만 해도 서버가 HTTP 200에 success:false로 내려줄지 4xx·5xx로 내려줄지부터 백엔드와 합의가 필요했고, 이 약속을 미리 맞춰두지 않으면 응답마다 처리 방식을 다시 논의하는 비용이 계속 쌓였습니다.
...
마이그레이션 자체는 팀이 AI로 주도했고, 본인 기여는 **그 결과물의 이미지 맞춤 부채를 식별·정리하고 회귀를 검증**(현재 진행)하는 부분입니다. (번들 절감 같은 전환 수치는 팀 차원의 성과로, 본 케이스의 정량 성과로 포함하지 않습니다.) 교훈은 분명합니다 — **결과 이미지를 맞추는 빠른 마이그레이션은 미세조정에 기대 취약하고, 소스 엔진의 동작을 이해해 타깃 엔진의 표준 기능으로 옮겨야 조건이 바뀌어도 어긋나지 않는다.**
...
다만 타입 체크가 만능은 아니라는 것도 뒤늦게 배웠습니다. 같은 방식을 본 앱에도 넓힐지 검토하며 그동안의 i18n 버그를 직접 집계해보니, 정작 가장 흔한 '번역 키 노출'은 영어 단일 소스로 만든 타입 체크가 잡지 못하는 부분이었습니다 — 언어별 키 누락이라, 더 싸고 정확한 건 언어 간 키 parity 체크였습니다. 도구는 좋아 보이는가가 아니라 무엇을 막는가를 데이터로 따져 선택해야 한다는 걸, 제가 도입했던 방법을 스스로 점검하며 확인했습니다.
...
> **성과** — 좌표계·전달 방식이 제각각인 외부 CAM 12종을 단일 연동 인터페이스로 통합, 앱마다 다르던 설치 감지 타임아웃을 3초로 맞추고 비차단 안내로 통일
...
Custom Protocol을 **중간 레이어**로 두어, 브라우저가 프로토콜로 Linker를 실행하며 세션 정보를 넘기면 Linker가 서버에서 파일을 직접 받아 변환 후 CAM을 실행하도록 재설계했습니다. 좌표계·전달 방식이 서로 다른 **외부 CAM 12종**(프로세스 실행 방식 8종 + 포트 연동 방식 4종)의 차이는 **하나의 변환·연동 인터페이스로 추상화**했습니다.
...
감지를 완벽하게 만들려 하지 않고, **틀려도 사용자가 막히지 않는** 쪽으로 정책을 통일했습니다. batch 2.5초·Linker 10초로 서로 다르던 감지 타임아웃을 **3초로 맞추고**, 3초 내 미감지면 미설치로 간주하되 **설치 안내를 항상 비차단(non-trapping)으로 표시해** 어떤 경우에도 다음 행동(다운로드)이 가능하게 했습니다. DevTools로 인한 false-positive는 타임아웃으로 막지 못하는 한계임을 **인지한 채 수용**했습니다.
...
먼저 컴포넌트를 **라이브러리로 배포**했지만 각 서비스가 버전업·재배포를 반복해야 해 문제가 그대로 남았습니다. 그래서 가장 빠르고 단순한 **iframe + postMessage** 런타임 통합으로 전환했습니다. 당시 모듈은 각자 도메인을 갖고 iframe으로 들어가, 배포도 모듈 수만큼 해야 했습니다.

모듈을 따로 배포하는 전제는 팀이 나뉘어 있을 땐 맞았습니다 — 모듈만 고치면 모듈만 배포하면 됐으니까요. 그런데 실제로는 모듈이 따로 배포되는 경우 대부분이 독립적인 UI 수정이 아니라, 주요 솔루션이 함께 쓰는 공용 라이브러리가 업데이트되며 같이 수정되는 경우였습니다. 이미 잘 도는 기능의 UI만 바뀌는 일은 드물었습니다. 4개 앱이 한 모노레포로 합쳐진 것과 맞물려, 어차피 같이 배포될 거라면 **빌드타임에 합쳐** 배포 단위를 줄이는 편이 나았습니다.

다만 빌드타임 통합은 통합 배포가 실제로는 잘 이뤄지지 않고 사내 배포 주기 제약으로 부담만 커졌습니다. 그래서 **다시 iframe으로 돌아오되**, 예전처럼 모듈마다 도메인을 두는 대신 **cloud 앱의 하위 라우트로 올려** cloud가 배포되면 함께 배포되도록 했습니다. same-origin이라 CORS·인증 공유·origin 검증도 단순해졌고, 모듈 수만큼 따로 배포하던 피로도 사라졌습니다.
...
> **성과** — 분리됐던 팀들이 합쳐지는 조직 변경에 맞춰 앱 2종·라이브러리 6종을 커밋 이력 보존하며 NX 모노레포로 이관하고, 버전이 어긋나던 공용 라이브러리를 빌드타임에 합쳐 통합 배포·버전 정합을 확보, 잔존 설정이 유발하던 OOM도 진단해 빌드 태스크 11→9로 정리

### 문제

분리됐던 팀들이 합쳐지는 조직 변경이 있었습니다. 각자 관리하던 레포가 떨어져 있으니 하나의 변경이 여러 PR·여러 배포로 나뉘었고, 공용 라이브러리는 앱마다 버전이 어긋나 배포 시점을 서로 맞추고 팀 간 버전을 조율하는 커뮤니케이션 비용이 컸습니다. 마침 통합 배포를 추진하게 되면서 그 구조에 맞게 레포 구조도 바꾸기로 했습니다. 환경별 URL·도메인도 각자 다르게 구성돼 있었고, 다국어 관리도 중복 업로드와 분산 스크립트로 파편화된 상태였습니다.
...
> **성과** — 명세 검증을 AI로 전량 자동화하려다 비결정성·비용의 한계를 겪고 격리 재현 위 E2E로 자리 잡았고, 정성 평가로는 멀쩡하던 테스트가 실제로는 결함을 잡지 못한다는 것을 mutation testing으로 확인한 뒤, 배포마다 검증을 통과해 나간 결함을 추적하는 CFR 대시보드를 직접 만들어 팀에서 운영 중

### 문제 — AI가 개발을 가속하자 회귀가 늘었다

AI 도입으로 코드 변경 속도가 빨라진 만큼 회귀도 함께 늘었습니다. 고친 버그가 다시 나오고, 한쪽 변경이 다른 쪽을 망가뜨리는 일이 배포마다 반복됐습니다. 제품에는 애초에 테스트가 거의 없었고, 그나마 있는 테스트 대부분은 AI 도입 이후 빠르게 만들어진 것이었습니다.

### 해결 — 여러 방법을 거친 진화

처음엔 기능 명세가 실제로 검증되는지를 e2e로 자동화하려 했습니다. 하지만 e2e를 돌릴 격리 환경도 셋업 코드도 없어 환경을 만드는 데 시간이 다 들어갔습니다. 다음으로 AI가 Playwright MCP로 브라우저를 직접 조작해 명세대로 검증하게 했습니다. 커버리지는 늘었지만 실행할 때마다 결과가 달라지는 비결정성이 문제였습니다. 검증 절차를 YAML로 형식화해 결정성을 확보하려 했지만, 이번엔 e2e 코드보다 실행 시간과 토큰 비용이 컸습니다. 결국 e2e 코드로 돌아왔고, 격리 재현 환경 위에서 결정론적으로 돌렸습니다.

격리 재현 환경은 본인이 만든 도메인 통합 라이브러리 위에서 특정 커밋 시점의 클라이언트·서버·DB를 그대로 Docker로 띄우는 구조입니다. 런타임 분리 자체는 팀이 했고, 본인은 그 위에 격리 환경을 올렸습니다.

![격리 재현 환경 — 도메인 통합·런타임 분리 위에서 커밋 시점의 클라이언트·서버·DB를 그대로 재현한다](images/portfolio/diagrams/isolation-stack.svg){.diagram}

그 위에서 분산돼 있던 E2E를 정리해 반복되던 로그인 셋업을 공통 헬퍼로 약 93% 줄이고 Page Object로 리팩토링했습니다. 8개 앱을 동시에 띄우면 S3 다운로드가 DNS를 포화시켜 실행이 실패하던 문제는, 처음엔 포트 충돌로 의심했지만 수정 유무를 대조 실행해 직접 반증하고 공유 캐시 프리페치로 해결했습니다. 그리고 "전체 E2E는 느리고 안 돌리면 회귀를 놓친다"는 딜레마를 두고, QA팀이 관리하는 수백 개 테스트 케이스를 Planner·Verifier 두 에이전트로 검증하던 자작 도구 TC-Verify를 확장해, 커밋 변경과 관련된 테스트만 AI가 골라 실행하고 실패를 실제 회귀와 테스트 코드 문제로 1차 분류하게 했습니다.

![AI 변경 감지 — 커밋 변경 분석에서 연관 테스트 선별, 격리 실행, 회귀와 테스트 문제 1차 분류로](images/portfolio/diagrams/ai-detect-pipeline.svg){.diagram}

### 그래도 못 잡았다 — 물러서서 '측정'부터

이렇게 했는데도 예정된 배포가 품질 기준을 통과하지 못해 밀렸습니다. 물러서서 보니, AI가 만든 테스트를 다시 AI가 평가하다 보니 겉으로는 멀쩡해 보여도 실제로는 결함을 잡지 못하는 테스트가 많았습니다. 이걸 mutation testing으로 측정하니, 정성 평가에서 'A급'을 받은 코어 라이브러리조차 mutation score가 57%였습니다. 심어 둔 결함의 43%를 테스트가 그대로 통과시킨 겁니다.

그래서 방향을 바꿔, 어떤 결함이 검증을 통과해 배포까지 갔는지부터 보이게 만들기로 했습니다. 배포마다 빠져나간 결함을 추적하는 CFR 대시보드를 직접 만들어 지금도 팀에서 씁니다. 단순 건수가 아니라 심각도를 함께 보고, 임의 기준선 대신 관리 한계로 정상 범위를 판단하며, 두 번 이상 발견된 재발 결함까지 한 화면에서 봅니다.
...
AI 변경 감지에도 한계가 있었습니다. 어떤 회귀는 오히려 AI 리뷰가 통과시킨 잘못된 테스트에서 나왔습니다 — 기획 의도가 코드에도 테스트에도 없이 티켓 코멘트에만 있었던 탓입니다. 직접 바로잡았고, AI를 판단에 쓸수록 그 판단이 맥락 없이 틀릴 수 있다는 것까지 함께 설계해야 한다는 걸 배웠습니다. EC2 상시 구동의 보안·안정성 부담을 줄이려 변경 감지는 이후 로컬 데일리 E2E로 옮겼습니다.
...
![빌드 파이프라인 재설계 성과 — 빌드 시간 33~39분에서 17~24분, 업로드 아티팩트 757MB에서 334MB로 약 56% 감소](images/portfolio/diagrams/build-metrics.svg){.diagram}
...
빌드를 **물리 빌드머신 + self-hosted GitHub Actions**로 옮기고, **Windows 코드 서명 에이전트를 빌드머신에 직접 설치·풀 등록**해 단독 운영하도록 만들었습니다. 서명 USB와 인프라팀 의존을 끊은 것입니다. macOS 파이프라인은 빌드 단계를 재설계해 **전체 33~39분 걸리던 빌드를 17~24분으로** 줄였고, 업로드 아티팩트도 757MB에서 334MB로 약 56% 줄였습니다.
