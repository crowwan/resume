...
2023.09부터 AI 기반 치과 CAD/CAM SaaS(DentBird)에서 화면 구현을 넘어 **제품의 구조와 품질을 책임지는** 프론트엔드 개발자로 일해왔습니다. 분산된 앱을 NX 모노레포로 통합하고 공통 모듈을 Micro Frontend로 분리하는 플랫폼 구조를 설계했고, 변경된 코드를 분석해 관련 E2E만 자동 실행하는 시스템과 커밋 단위 격리 재현 환경으로 릴리스 신뢰성을 끌어올렸습니다. Electron 데스크톱 앱을 0→1로 설계하고 빌드를 20분에서 6분으로 줄이는 등, FE를 중심으로 제품의 처음부터 끝까지 다룹니다.
...
|------|------|
| **기간** | 2024.06 ~ 현재 |
| **역할** | 런타임 환경 분리 공동 설계 · 격리 재현/E2E 자동화 주도 |
...
- 원인이 환경변수를 **빌드타임에 박아 넣는 구조**임을 진단하고, 환경변수를 **런타임에 주입**하는 구조로 전환하는 작업을 팀과 공동 설계했습니다. 같은 빌드 결과물 하나를 dev·qa·prod에 그대로 올리고 환경변수만 교체하도록 만들어, 환경별 빌드 시간을 줄이고 배포 직후 핫픽스 반복을 해소했습니다.
...
| **역할** | 빌드·배포·자동 업데이트 파이프라인 설계·이관 주도 (Batch·Linker 공통 인프라) |
...
- Azure Pipelines가 클라우드 러너라 빌드가 20분 넘게 걸리는 문제를, **GitHub Actions + self-hosted 러너로 이관 주도**해 해결했습니다(#7689). 영구 캐시 경로로 BrokerServer 부하를 회피하고 Phase 단위로 단계적 이관 후 macOS universal·버전 전파 회귀 등을 안정화했습니다. 그 결과 빌드 시간을 **20분 → 6분(직접 비교)** 으로 줄이고 담당자·인프라팀 의존을 제거했습니다.
...
| 영역 | 지표 | 수치 |
|------|------|------|
| 빌드 인프라 | Electron 빌드 시간 (Azure 러너 대비, 직접 비교) | 20분 → 6분 |
