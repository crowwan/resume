...
**Phase: 마스터 안정화 + 회사별 파생 운영 중. 오늘의집 서류 탈락(2026-06)을 분기점으로 전략 전환 — 마스터=풀 카탈로그 / 파생=선별·압축·재배치(`applications/README.md` 체크리스트화). 포지셔닝은 "빠른 변화 속 안전한 개발"로 확정(메모리 `resume-positioning-safe-change`). 다음 = 라인·채널톡·토스플레이스 portfolio 파생 + 미해결 확인 3건.**

**문서 아키텍처 (2축, 확정)**
- `resume.md` (루트) → 담백 마스터 이력서 — ✅ 경력 섹션 신설·카드 8개·기술스택 정정 완료
- `portfolio.md` (루트) → 서사 케이스 11개 (개요/문제/해결/회고 통일, 코드블럭 제거) — ✅ 안정
- `archive/career-description.md` → 보존 (빌드·index 제외) — ✅
- `applications/README.md` → **JD 파생 체크리스트** (오늘의집 탈락 분석에서 도출) — ✅ 신설
- `applications/*/` → 회사별 파생: **오늘의집·당근 완비**(resume+portfolio+research) / 라인(answers형)·채널톡·토스플레이스(resume만) — 🔶 portfolio 3사 미완

원칙: resume는 "무엇을 했나(담백)", portfolio는 "왜·어떻게·무엇을 배웠나(서사)". JD 변형은 **내용 재작성이 아니라 Case 선별·순서·강조(큐레이션)**. 파생은 마스터에 있는 사실만, 새 수치 발명 금지. 카드 추상화 = "판단 보이는가 + 오귀속 안 하는가"(메모리 `resume-architecture-and-card-abstraction`).
