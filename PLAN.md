...
**Phase: 첨삭 기준 체크리스트 신설 + Medit 인앱 신규 카드 + 파생 6사 동기화 완료. 최근 6주(6/10~7/21) 세션·볼트 재발굴로 2026 구간 FE 깊이 보강, 자기소개 결제 예시 제거·도입문 압축, longevity 프레임을 safe-change 축으로 정리. 다음 = (선택) i18n 도메인 어휘 SSoT·테스트품질(Stryker) 카드 승격 · 미해결 확인 3건 · `resume/medit-and-6sync` 브랜치 main 머지 → Pages 배포.**
...
## 이번 구간 (2026-07-06 — 성과 중심 강화·표현 정리·파생 재큐레이션) 완료

> ★ 외부 "성과 중심 경력기술서" 제안서 2건 검토 → 발명 위험(수치 역추적·GPT KPI·3C4P)은 배제하고 유효한 축(How+Result·성과 헤드라인·정량 시각화)만 흡수. 구어체·AI문어투·출처과시 표현을 전수 정리하고 재발방지 규칙을 3곳(WRITING-GUIDE·CLAUDE.md·메모리)에 박제.

| 작업 | 상태 |
|------|------|
| resume 주요 프로젝트 8카드 도입문에 정량 성과 반영(별도 줄 X, 도입 문장에 녹이는 B안) | ✅ |
| portfolio 정량 성과 시각화 4종 SVG — 세로막대(빌드)·가로타임바(3D)·도넛(계정구독)·숫자카운터(격리). 메인색(indigo) 통일, before 빨강 | ✅ |
| "PR 실측"·"소비처 실측(grep)" 등 출처 과시 표현 전 문서 제거 | ✅ |
| 구어체(깨지다·걷어내다·묶다·씨름·매달리다·내가) + AI문어투(오판·흡수·수렴·체득·저울질) 교정 | ✅ |
| **재발방지 박제** — WRITING-GUIDE §4-7·9 + CLAUDE.md §6·§8(수정 전·후 grep 셀프체크 16패턴) + 메모리 `resume-voice-preference` | ✅ |
| 파생 6사 교정 동기화 + 오늘의집·당근·에이블리 resume 도입문 성과 + 당근·에이블리 portfolio 시각화 | ✅ |
| **채널톡·토스플레이스 파생 최신 2축 재큐레이션** — 옛 구조→마스터 최신 카드, portfolio 신규 생성(채널톡=제품·판단 먼저, 토스=Linker 톱) | ✅ |

> 오늘의집 portfolio는 성과 헤드라인 없는 옛버전이나 **서류 탈락 확정으로 스킵**(사용자 결정). 재지원 시 최신화.

## 이번 구간 (2026-07-21 — 첨삭 기준·Medit 카드·재발굴·6사 동기화) 완료

> ★ 외부 첨삭 기준(신입 취준용) 검토 → 경력직 렌즈로 필터링해 체크리스트 신설. 최근 6주(6/10~7/21) 세션+볼트 재발굴로 Medit 인앱 신규 카드 확보. 마스터 다듬고 파생 6사 동기화.

| 작업 | 상태 |
|------|------|
| **REVIEW-CHECKLIST.md 신설** — 외부 신입 첨삭 기준을 경력직용으로 필터(증명사진·신입 명기·가상 트래픽 수치 제외/경고). CLAUDE.md §9 등록 | ✅ |
| **자기소개 정리** — 결제 28→0 예시 제거(질문 유도력 낮다는 본인 판단), AI 검증 통제 문장 선명화 | ✅ |
| **도입문 압축** — 계정구독·3D·결제 등 정보밀도 높던 카드 6초 스캔 개선(마스터 + 파생) | ✅ |
| **세션+볼트 재발굴**(에이전트 2기 병렬, git 교차검증) — 2026-06-10 이후 신규 구간. Medit·i18n SSoT·Batch 컷오버·MillBox 등 후보 도출 | ✅ |
| **Medit 인앱 카드 신설** — 마스터 resume(담백)+portfolio(서사). 협력사 Electron 셸 현대화(엔진 25→31·JSPI·MUI 전환·로그인 IPC 재설계). 귀속=셸 파트(C++ 제외), 결과 정성적(임의 수치 X) | ✅ |
| **카드7 Batch 멀티플레이버 컷오버 보강** — per-variant 커스텀 프로토콜(정식·실험 딥링크 충돌 해소) | ✅ |
| **파생 6사 동기화** — 당근·에이블리 자기소개 결제 제거, 도입문 압축, 토스·채널톡 Medit 카드 추가, 파생 html 전부 pandoc 재생성 | ✅ |
| **longevity 프레임 정리** — 전 portfolio Linker 회고 "오래 버티는 방법" → "정책이 또 바뀌어도 흔들리지 않는 방법"(safe-change 축, 메모리 `resume-positioning-safe-change`) | ✅ |

> 발굴 후보 중 미채택: i18n 도메인 어휘 SSoT·테스트품질(Stryker)·dedup(측정 우선) — 강하나 이번 미반영(재지원/보강 시 승격, git author·발명 재확인 필요). 자기소개 훅은 이 구간에 정량 클린 케이스가 없어 성향 중심 유지.
> 커밋: 브랜치 `resume/medit-and-6sync`에 논리 3커밋(첨삭 체크리스트 / 마스터 콘텐츠 / 파생 동기화) + longevity·Medit 후속. main 미머지(배포 대기).

## 다음 세션 할 일 (우선순위)

| # | 작업 | 메모 |
|---|------|------|
| 1 | **미해결 확인 3건 처리** | 아래 "미해결 확인사항" — Case i18n 프로덕션 반영 여부 · "격리 환경 직접 구축" 경계 · CAM 12종 표현 (발명 방지) |
| 2 | **라인 resume.md 본체 여부 결정** | 현재 `answers.md`(5문항 답변형)+style만 있고 이력서 본체 없음 — 답변형 유지할지/별도 이력서 낼지 사용자 확인 |
| 3 | (선택) **백로그 케이스 승격** | `findings/portfolio-case-backlog.md` — Feature Flag·Mock Server·TC-Verify 등. 마스터 11케이스로 안정이라 우선순위 낮음. 승격 시 git author 검증 필수 |
| 4 | (선택) **오늘의집 portfolio 최신화** | 재지원 시 성과 헤드라인 + 시각화 반영 (현재 옛버전) |
