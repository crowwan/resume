...
- **checksum mismatch (개발 중 학습, ✅ 사용자 정정 2026-06-01)**: 코드사인 없이 빌드한 아티팩트를 **이후에 따로 코드사인**하면 바이너리 checksum이 바뀌어 latest.yml의 sha512와 불일치 → 자동 업데이트 실패 `[565282665]`. 당시 sha512 재계산으로 임시 대응.
  - ★ **실제 배포는 코드사인을 빌드 과정에 포함**해 이 문제가 발생하지 않음. "해결한 기능"이 아니라 **코드사인을 빌드에 통합해야 한다는 근거**로 학습된 점 (→ 빌드 파이프라인 자동화 §1과 연결)
