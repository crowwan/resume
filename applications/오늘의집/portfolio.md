...
테스트·디버깅의 신뢰성 문제를, 테스트가 믿을 수 있게 돌아가는 **재현 가능한 환경을 만드는 방향**으로 푼 프로젝트입니다. 선행 구조(Case 3의 도메인 통합 + 팀의 런타임 분리)부터 쌓고, 그 위에 격리 재현 환경을 구축하고, 다시 그 위에 **"바뀐 코드만 골라 테스트하는" AI 변경 감지**까지 구축했습니다.
...
원인은 **빌드 시점에 환경변수가 번들에 박히는 구조**였습니다. 런타임 분리 자체는 팀이 진행했고, 본인은 그 토대로 **도메인 통합 라이브러리(Case 3)**를 구축했습니다. 환경이 런타임에 결정되니, **특정 커밋 시점으로 Docker를 기동해 그때의 클라이언트 + 서버 + DB를 그대로 재현**하는 격리 환경을 쌓을 수 있었습니다.
