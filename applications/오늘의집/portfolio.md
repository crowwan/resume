...
원인은 **빌드 시점에 환경변수가 번들에 고정되는 구조**였습니다. 런타임 분리 자체는 팀이 진행했고, 본인은 그 토대로 **도메인 통합 라이브러리(Case 3)**를 구축했습니다. 환경이 런타임에 결정되니, **특정 커밋 시점으로 Docker를 기동해 그때의 클라이언트 + 서버 + DB를 그대로 재현**하는 격리 환경을 구축할 수 있었습니다.
