...
2023년 9월부터 AI 기반 치과 CAD/CAM SaaS인 Dentbird의 프론트엔드를 맡아, 화면 구현부터 모노레포·환경 분리·품질 자동화 같은 플랫폼 토대까지 폭넓게 다뤄 왔습니다. 문제를 빠르게 덮기보다 근본 원인을 이해하고 재현으로 확인하며, 트레이드오프를 따져 도입을 결정하는 방식으로 일합니다.
...
- 썸네일을 IntersectionObserver 배치 lazy-loading으로 처리하고, 성공·실패를 함께 캐시해 **재마운트 시 중복·무한 재요청을 제거**하고 회귀 테스트로 고정
- 깜빡임·무한 재요청 회귀의 근본 원인인 **카드 재마운트마다 훅이 새로 생기는 구조**를 진단해, 캐시를 공유 스코프로 끌어올려 구조적으로 해결
